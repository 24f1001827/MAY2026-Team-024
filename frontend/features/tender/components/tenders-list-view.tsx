...
/**
 * Presentational tenders list (read-only) — rows link to the linked complaint,
 * where the tender is managed. Shared by the officer (own tenders) and admin
 * (all tenders) views.
 */
export function TendersListView({
  ...
  tenders: OfficerTenderListItem[] | undefined
  isPending: boolean
  isError: boolean
  error: unknown
  refetch: () => void
  ...
  const filtered = useMemo(() => {
    ...
    return tenders.filter(
      (t) =>
        ...
        t.complaintTitle.toLowerCase().includes(q),
    )
  }, [tenders, search])
          ...
          <CardContent className="p-0">
            {isPending ? (
              ...
              <Empty className="py-12">
                ...
                <EmptyHeader>
                  <EmptyTitle>No tenders yet</EmptyTitle>
                  <EmptyDescription>
                    {tenders.length === 0 ? emptyHint : "Try adjusting your search."}
                  </EmptyDescription>
                </EmptyHeader>
              </Empty>
            ) : (
                ...
                <TableBody>
                  {rows.map((t) => (
                    <TableRow
                      ...
                      onClick={() =>
                        router.push(routes.complaints.detail(t.complaintId).href)
                      }
                    >
                      ...
                      <TableCell className="max-w-xs text-sm text-muted-foreground">
                        <span className="line-clamp-1">{t.complaintTitle}</span>
                      </TableCell>
